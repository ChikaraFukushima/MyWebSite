...
/*入力内容を確認 バリデーションエラーがある場合新規登録画面に遷移(復習ポイント) */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 入力されているパスワードが確認用と等しいかチェックする
			if (!inputPassword.equals(inputConfirmPassword)) {
				validationMessage += "入力されているパスワードと確認用パスワードが違います<br>";
			}

			// ログインIDの入力規則チェック 英数字 ハイフン _のみ入力可能
			...
			if (UserDAO.isOverlapLoginId(udb.getLoginId(), 0)) {
				validationMessage += "現在ほかのユーザーが使用中のログインIDです";
			}
